...
if [ "$ENCODING" = "interCRT100" ]; then
    ...
    DATA_TYPES='int[201]:range(-1,2)'
    BASE_FILENAME="${FUNC_PREFIX}_interCRT_sqfree"
else
...
$PYTHON_BIN ../../Int2Int/train.py --seed 42 --env_base_seed 100 --num_workers 0 --dump_path "`abspath ${MODEL_DIR}`" --exp_name $EXP_NAME --exp_id 1 --train_data "`abspath ${INPUT_DIR}/${TRAIN_FILE}`" --eval_data "`abspath ${INPUT_DIR}/${EVAL_FILE}`" --eval_size 10000 --epoch_size 50000 --operation data --data_types "$DATA_TYPES" --optimizer 'adam_inverse_sqrt,lr=0.00025' --max_epoch 201 --batch_size 128 --validation_metrics _valid_arithmetic_xe_loss,valid_arithmetic_acc --dropout 0.1 --attention_dropout 0.1
