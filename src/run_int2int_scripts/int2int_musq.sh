...
if [ "$ENCODING" = "interCRT100" ]; then
    ...
    TRAIN_FILE="musq_modp_and_p.txt.train"
    EVAL_FILE="musq_modp_and_p.txt.test"
elif [ "$ENCODING" = "CRT100" ]; then
    DATA_TYPES='int[100]:range(-1,2)'
    TRAIN_FILE="musq_CRT100.txt.train"
    EVAL_FILE="musq_CRT100.txt.test"
elif [ "$ENCODING" = "interCRT100_with_n" ]; then
    DATA_TYPES='int[201]:range(-1,2)'
    TRAIN_FILE="musq_interCRT100_with_n.txt.train"
    EVAL_FILE="musq_interCRT100_with_n.txt.test"
elif [ "$ENCODING" = "CRT100_with_stats" ]; then
    DATA_TYPES='int[103]:range(-1,2)'
    TRAIN_FILE="musq_CRT100_with_stats.txt.train"
    EVAL_FILE="musq_CRT100_with_stats.txt.test"
else
